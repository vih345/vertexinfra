...
if ($method === 'POST' && $action === 'create') {
    ...
    if ($name === '' || $message === '' || strlen($name) > 240 || strlen($message) > 1680) respond(['error' => 'Dados inválidos.'], 422);
...
if ($method === 'PATCH') {
    $body = json_decode((string) file_get_contents('php://input'), true);
    $message = trim((string) ($body['message'] ?? ''));
    if ($message === '' || strlen($message) > 1680) respond(['error' => 'Comentário inválido.'], 422);
